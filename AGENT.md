...
> 3. `QUALITY.md` — Standar Kualitas & Spesifikasi Anti-AI-Slop (*Anti-AI-Slop Specification*)  
> 4. `AGENTS.md` / `AGENT.md` / `GEMINI.md` / `CLAUDE.md` — Standar Rekayasa & Protokol Eksekusi Agent  
...
Agen memegang prinsip bahwa kode yang dihasilkan harus berstandar produksi (*production-grade*), bersih, hemat sumber daya (*zero bloat*), aman, menolak pola *AI Slop* (`QUALITY.md`), dan mematuhi kedaulatan data lokal (*local-first*).
...
4. **KONSISTENSI DESIGN SYSTEM (`DESIGN.md` & `QUALITY.md`):**
   ...
   - Patuhi spesifikasi anti-AI-slop di `QUALITY.md` (hindari 3-card grid berulang, blur/glassmorphism berlebihan, buzzwords generik).

5. **EKSEKUSI BERTAHAP (1 FASE $\rightarrow$ REVIEW $\rightarrow$ LANJUT):**
   - **DILARANG** membangun seluruh website sekaligus dalam satu langkah prompt (*"build entire platform"*).
   - Wajib mengikuti alur 10 Fase: **Phase 1 (Foundation) $\rightarrow$ Phase 2 (Homepage) $\rightarrow$ Phase 3 (Products) $\rightarrow$ Phase 4 (Product Detail) $\rightarrow$ Phase 5 (Downloads) $\rightarrow$ Phase 6 (AI Lab) $\rightarrow$ Phase 7 (Projects) $\rightarrow$ Phase 8 (Docs) $\rightarrow$ Phase 9 (Polish) $\rightarrow$ Phase 10 (Quality Audit)**.
   - Selesaikan satu fase, berikan laporan, tunggu review/persetujuan sebelum melanjutkan ke fase berikutnya.
...
```
1. INSPECT     ──> Baca file terkait, skema, PRD.md, DESIGN.md, dan QUALITY.md
...
6. VERIFY      ──> Pastikan tidak ada regresi performa, visual, & AI slop
7. REVIEW      ──> Konfirmasi kepatuhan penuh terhadap PRD, DESIGN, & QUALITY
```
...
├── QUALITY.md             # Quality Assurance & Anti-AI-Slop Specification
├── GEMINI.md              # Gemini Agent Execution Directives
├── CLAUDE.md              # Claude Agent Execution Directives
└── AGENT.md / AGENTS.md   # Universal AI Agent Directives (File Ini)
```
...
## 6. Format Commit & Pesan Riwayat (Git Hygiene)
